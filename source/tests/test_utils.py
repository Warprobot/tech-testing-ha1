...
class UtilsTestCase(unittest.TestCase):
    ...
    def test_writefile_exception(self):
        ...
        with patch('source.lib.utils.open', Mock(side_effect=IOError("Can't write to a file")), create=True) as m_open:
    ...
    def test_wrong_config_filepath(self):
        with self.assertRaises(IOError):
            load_config_from_pyfile('wrong/path')
    ...
    def test_load_configfile_success(self):
        ...
        with patch('source.lib.utils.exec_pyfile', Mock(return_value=variables)):
            var = load_config_from_pyfile(os.path.realpath(os.path.expanduser("source/tests/test_config_ok")))
    ...
    def test_load_configfile_fail(self):
        ...
        with patch('source.lib.utils.exec_pyfile', Mock(return_value=variables)):
            returns = load_config_from_pyfile(
    ...
    def test_parse_cmd_args_exist(self):
        ...
        parser = Mock()
        with patch('source.lib.utils.argparse.ArgumentParser', Mock(return_value=parser)):
            parse_cmd_args(parameters)
    ...
    def test_spawn_workers(self):
        args = []
        num = 10
        with patch('source.lib.utils.Process', Mock()) as mock_process:
            spawn_workers(num, "target", args, num)
    ...
    def test_network_status_ok(self):
        ...
        with patch('urllib2.urlopen', Mock(return_value=True)) as urllib:
            check_network_status(check_url, timeout)
    ...
    def test_network_status_fail(self):
        ...
        with patch('urllib2.urlopen', Mock(side_effect=ValueError("network status fail"))):
            self.assertFalse(check_network_status(check_url, timeout))
